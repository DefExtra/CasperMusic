...
const commandName = "leave-empty";
class SetupCommand extends BaseCommand_1.default {
    constructor() {
        super(commandName, "admin", ["le"], {
            name: commandName,
            description: "leave when the queue is empty.",
            type: "CHAT_INPUT",
            options: [],
        });
    }
    run(client, interaction, message, args, author) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let data = quick_db_1.default.fetch(`LE_${base.guildId}`);
            if (data == null) {
                yield quick_db_1.default.set(`LE_${base.guildId}`, true);
                if (message == null)
                    interaction
                        .editReply({
                        content: replys.leaveOnEmpty.replace("{status}", String(true)),
                    })
                        .catch(() => { });
                else
                    message
                ...
                yield quick_db_1.default.set(`LE_${base.guildId}`, false);
                if (message == null)
                    interaction
                        .editReply({
                        content: replys.leaveOnEmpty.replace("{status}", String(false)),
                    })
                        .catch(() => { });
                else
                    message
